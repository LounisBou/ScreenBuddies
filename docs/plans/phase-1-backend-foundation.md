...
```
FOR EACH sub-phase in order (1.1 → 1.2 → ... → 1.9):
...
1. **Unit Tests**: Test individual model methods, services, and helpers
2. **Feature Tests**: Test API endpoints, database operations, and user stories
3. **Coverage Target**: 100% code coverage
...
1. **Create a feature branch**
   ...
   # Example: git checkout -b feat/1.2-user-models
   ```

2. **Make small commits** with clear, descriptive messages
   ```bash
   git commit -m "feat: create User model with migration"
   git commit -m "test: add User model unit tests"
   git commit -m "feat: add UserPreference relationship"
   ```
...
|-----------|------|-------------|------------------|
| **1.1** | [`phase-1/1.1-sanctum-pest-setup.md`](./phase-1/1.1-sanctum-pest-setup.md) | Sanctum & Pest Setup | Token auth, testing framework |
| **1.2** | [`phase-1/1.2-user-models.md`](./phase-1/1.2-user-models.md) | User Models | User, UserPreference + tests |
| **1.3** | [`phase-1/1.3-media-type-model.md`](./phase-1/1.3-media-type-model.md) | MediaType Model | Media categorization + tests |
| **1.4** | [`phase-1/1.4-friendship-model.md`](./phase-1/1.4-friendship-model.md) | Friendship Model | Social relationships + tests |
| **1.5** | [`phase-1/1.5-election-model.md`](./phase-1/1.5-election-model.md) | Election Model | Core election entity + tests |
| **1.6** | [`phase-1/1.6-candidate-model.md`](./phase-1/1.6-candidate-model.md) | Candidate Model | Election items + tests |
| **1.7** | [`phase-1/1.7-voter-model.md`](./phase-1/1.7-voter-model.md) | Voter Model | Vote storage (JSON blob) + tests |
| **1.8** | [`phase-1/1.8-user-relationships.md`](./phase-1/1.8-user-relationships.md) | User Relationships | Complete User model + tests |
| **1.9** | [`phase-1/1.9-seeders-finalization.md`](./phase-1/1.9-seeders-finalization.md) | Seeders & Finalization | MediaType seeder, verification |

---

## Phase 1 Completion Criteria

All items MUST be verified before proceeding to Phase 2:
...
- [ ] All relationships configured with tests
- [ ] Vote JSON validation in Voter model with tests
...
After all sub-phases complete and checklist verified → Proceed to **Phase 2: Authentication & User Management**
