...
```
FOR EACH sub-phase in order (2.1 → 2.2 → ... → 2.9):
...
1. **Unit Tests**: Test services, helpers, validation logic
2. **Feature Tests**: Test API endpoints with all scenarios (success, validation errors, auth errors)
3. **User Story Tests**: Test complete user flows (register → verify email → login → use features)
...
1. **Create a feature branch**
   ...
   # Example: git checkout -b feat/2.3-login
   ```

2. **Make small commits** with clear, descriptive messages
   ```bash
   git commit -m "feat: create LoginRequest with validation"
   git commit -m "feat: implement AuthService login method"
   git commit -m "test: add login endpoint feature tests"
   ```
...
|-----------|------|-------------|------------------|
| **2.1** | [`phase-2/2.1-api-error-handling.md`](./phase-2/2.1-api-error-handling.md) | API Error Handling | ApiException, validation errors + tests |
| **2.2** | [`phase-2/2.2-registration.md`](./phase-2/2.2-registration.md) | User Registration | POST /auth/register + tests |
| **2.3** | [`phase-2/2.3-login.md`](./phase-2/2.3-login.md) | User Login | POST /auth/login + tests |
| **2.4** | [`phase-2/2.4-token-management.md`](./phase-2/2.4-token-management.md) | Token Management | POST /auth/refresh, /auth/logout + tests |
| **2.5** | [`phase-2/2.5-user-profile.md`](./phase-2/2.5-user-profile.md) | User Profile | GET/PUT /me + tests |
| **2.6** | [`phase-2/2.6-password-management.md`](./phase-2/2.6-password-management.md) | Password Change | PUT /me/password + tests |
| **2.7** | [`phase-2/2.7-friendships.md`](./phase-2/2.7-friendships.md) | Friendships | CRUD /friends + tests |
| **2.8** | [`phase-2/2.8-email-verification.md`](./phase-2/2.8-email-verification.md) | Email Verification | Verification flow + tests |
| **2.9** | [`phase-2/2.9-finalization.md`](./phase-2/2.9-finalization.md) | Finalization | Full test suite, coverage verification |
...
| POST | /api/v1/friends | Yes+V | Send friend request |
...
All items MUST be verified before proceeding to Phase 3:

- [ ] API exception handling configured
- [ ] Register/Login/Logout working with tests
- [ ] Token refresh working with tests
- [ ] User profile CRUD with tests
- [ ] Password change with tests
- [ ] Friendship system complete with tests
- [ ] Email verification working with tests
...
After all sub-phases complete and checklist verified → Proceed to **Phase 3: Election System**
